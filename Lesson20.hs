...
meanTS (TS _ []) = Nothing
meanTS (TS times values) | all (==Nothing) values = Nothing
                         | otherwise = Just avg
...
compareTS func (TS times values) | all (==Nothing) values = Nothing
                                 | otherwise = Just best
...
meanMaybe vals | any (==Nothing) vals = Nothing
               | otherwise = Just avg
               where avg = mean (map fromJust vals)

movingAvg :: (Real a) => [Maybe a] -> Int -> [Maybe Double]
movingAvg [] n = []
movingAvg vals n | length nextVals == n = meanMaybe nextVals:movingAvg restVals n
                 | otherwise = []
